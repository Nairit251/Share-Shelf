...
INSERT INTO `purchase_item` (`Purchase_Item_ID`, `Purchase_ID`, `Item_ID`, `Quantity`, `Unit_Price`) VALUES
(1, 1, 1, 1, 35000.00),
(2, 2, 2, 1, 18000.00),
(3, 3, 5, 1, 450.00),
(4, 4, 8, 1, 800.00),
...
INSERT INTO `support_ticket` (`Ticket_ID`, `User_ID`, `Subject`, `Message`, `Status`, `Admin_ID`) VALUES
(1, 5, 'Unable to Login', 'I forgot my password and cannot access my account.', 'Resolved', 47),
(2, 18, 'Claim Not Updating', 'My claim status has not changed for several days.', 'Pending', NULL),
(3, 24, 'Payment Issue', 'Payment was deducted but not reflected in my purchase.', 'Resolved', 47),
(4, 31, 'Edit Item', 'I cannot update my item information after posting.', 'Resolved', 48),
(5, 39, 'Pickup Time Change', 'I would like to reschedule the pickup time.', 'Pending', NULL),
(6, 42, 'Report Follow-up', 'I would like an update regarding my submitted report.', 'In Progress', 49),
(7, 47, 'Image Upload Error', 'Images fail to upload while creating a listing.', 'Resolved', 48),
(8, 50, 'Account Verification', 'Please verify my account so I can list more items.', 'Pending', NULL);
...
ALTER TABLE `item`
  ...
  ADD KEY `fk_item_admin` (`Admin_ID`);
...
ALTER TABLE `support_ticket`
  ...
  ADD KEY `User_ID` (`User_ID`),
  ADD KEY `fk_ticket_admin` (`Admin_ID`);
...
ALTER TABLE `purchase_item`
  ...
  ADD CONSTRAINT `purchase_item_ibfk_2` FOREIGN KEY (`Item_ID`) REFERENCES `item` (`Item_ID`);
...
ALTER TABLE `report`
  ...
  ADD CONSTRAINT `report_ibfk_2` FOREIGN KEY (`Item_ID`) REFERENCES `item` (`Item_ID`);
